...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        branches = [totals_tree(end(side(m))) for side in [left, right]]
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) and label(t) == find_value:
        ...
        branches = [replace_leaf(b, find_value, replace_value) \
                    for b in branches(t)]
...
def preorder(t):
    ...
    [2, 4, 6]
    """
    if branches(t) == []: # Same as is_leaf(t)
        return [label(t)]
    flattened_branches = []
...
def preorder_alt(t):
    return reduce(add, [preorder(child) for child in branches(t)], [label(t)]) 
"""
