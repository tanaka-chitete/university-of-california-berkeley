...
def balanced(m):
    ...
    # Iterate through mobile from 1 to -1 so [left, right] and not 
    # ['mobile', left, right'] and use recursion to get total weights of all
    # that hangs from both arms (as the depth is unknown)
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) and (label(t) == find_value): 
        ...
        return tree(label(t), [replace_leaf(b, find_value, replace_value) \
                               for b in branches(t)])
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) > 0, "Interval spanning 0 cannot be used as divisor" 
